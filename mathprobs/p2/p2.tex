...
\begin{enumerate}[label=(\alph*)]
    ...
    $\theta^{(t)} = \bigl(\mu_0^{(t)},\, \mu_1^{(t)},\, \pi^{(t)}\bigr)$,
    ...
    \begin{align*}
        \mathbb{E}_{\theta^{(t)}}\!\left[\ell(Y, Z;\, \theta) \;\middle|\; Y\right].
    \end{align*}

    \medskip
    \noindent\textit{M-step.}
    Maximize the expected complete-data log-likelihood with respect to $\theta$ and
    show that the closed-form update equations are
    \begin{align*}
        \pi^{(t+1)}
        &\;=\;
        \frac{1}{n}\sum_{i=1}^n w_i^{(t)}, \\[6pt]
        \mu_0^{(t+1)}
        &\;=\;
        \frac{\displaystyle\sum_{i=1}^n \bigl(1 - w_i^{(t)}\bigr)\,Y_i}
             {\displaystyle\sum_{i=1}^n \bigl(1 - w_i^{(t)}\bigr)}, \\[6pt]
        \mu_1^{(t+1)}
        &\;=\;
    ...
    the expected complete-data log-likelihood is not sufficient.
\end{enumerate}
\end{document}